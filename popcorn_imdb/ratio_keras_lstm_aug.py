...
concat_train_df = pd.concat([original_concat_df,removed_SBAR_pos, removed_SBAR_neg, removed_PP_pos, removed_PP_neg]).reset_index(drop=True)
...
print('Train...')
hist = model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=1, batch_size=64, verbose=1, callbacks=[es, mc])
